...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Информация = НСтр("ru = 'Модифицированная обработка для выгрузки списка аттркционов на сайт.'");
	ПараметрыРегистрации.Вид = ДополнительныеОтчетыИОбработкиКлиентСервер.ВидОбработкиДополнительнаяОбработка();
	ПараметрыРегистрации.Версия = "2025.04";
//	ПараметрыРегистрации.БезопасныйРежим = Ложь;

	Команда = ПараметрыРегистрации.Команды.Добавить();
	Команда.Представление = НСтр("ru = 'Настройки выгрузки атракционов на сайт'");
...
//	ОбновлениеИнформационнойБазыСлужебный.Пауза(4);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.Фото) Тогда
			ДанныеФото = РаботаСФайлами.ДанныеФайла(Выборка.Фото);
			ИмяФайла = Строка(Выборка.Фото.УникальныйИдентификатор()) + ДанныеФото.ИмяФайла;
			ДвоичныеДанные = ПолучитьИзВременногоХранилища(ДанныеФото.СсылкаНаДвоичныеДанныеФайла);
...
//	// Архивируем
//	// TODO Удалить паузу
//	ОбновлениеИнформационнойБазыСлужебный.Пауза(3);
